...
def ventas_cliente():
  ...
  if len(data)>0:
    ...
    table_header = ["Id", "Id Decsa", "Num. Cliente", "Cve Cte Mayorista", "Negocio", "Cedis", "Ruta de Venta", "Canal Estrat.", "Segmento", "GEC", "Condicion"]
    begin = yearStart
    end = int(yearEnd)+1
...
def ventas_operaciones():
  ...
  if len(data)>0:
      cell_hasta = len(data) + 7
      # add column headings. NB. these must be strings
      header = ["Zona", "ANO", "SEM", "Localidad", "Ruta", "Modalidad de venta", "Cajas - pedidas", "Cajas - entregadas", "HTL - Pedidos", "HTL - Entregados", "% de eficiencia"]
      ws.append(header)
